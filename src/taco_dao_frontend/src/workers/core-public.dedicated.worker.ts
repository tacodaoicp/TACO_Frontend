/**
 * Core Public Data Dedicated Worker (Fallback for SharedWorker)
 *
 * This is a dedicated worker version for browsers that don't support SharedWorker.
 * It imports the same logic but adapts it for the dedicated worker API.
 */
...
function checkIdleStatus(): void {
  ...
  if (isIdle && !wasIdle) {
    console.log('[CoreWorker-Dedicated] Entering idle mode - pausing refreshes')
  } else if (!isIdle && wasIdle) {
    console.log('[CoreWorker-Dedicated] Exiting idle mode - resuming refreshes')
  }
}
...
function recordActivity(): void {
  ...
  if (isIdle) {
    isIdle = false
    console.log('[CoreWorker-Dedicated] Activity detected - resuming refreshes')
  }
}
...
async function init(): Promise<void> {
  console.log('[CoreWorker-Dedicated] Initializing...')
  ...
  try {
    ...
    console.error('[CoreWorker-Dedicated] Error loading cache:', error)
  }
  ...
  try {
    ...
    console.error('[CoreWorker-Dedicated] Failed to create agent:', error)
    throw error
  }
...
function handleMessage(message: WorkerRequest): void {
  switch (message.type) {
    ...
    case 'SET_VISIBILITY':
      ...
      // When tab becomes visible, record activity to exit idle mode
    ...
    case 'RESET':
      ...
      console.log('[CoreWorker-Dedicated] RESET received - clearing backoff, queue, fetch count, and re-sending cache')
      backoff.resetAll()
...
async function handleSetNetwork(message: WorkerRequest): Promise<void> {
  ...
  try {
    ...
    console.error('[CoreWorker-Dedicated] Error clearing cache:', err)
  }
...
async function processSingleFetch(item: { dataKey: DataKey; retryCount: number }): Promise<void> {
  try {
    ...
    console.error(`[CoreWorker-Dedicated] Error fetching ${item.dataKey}:`, error)
...
async function fetchData(dataKey: DataKey): Promise<void> {
  ...
  switch (dataKey) {
    case 'cryptoPrices':
      data = serializeForTransfer(await fetchCryptoPricesData())
      break
    ...
    case 'tokenDetails':
      ...
      await setCached('totalTreasuryValueInUsd', dataStates.get('totalTreasuryValueInUsd')!)
      break
    ...
    case 'totalTreasuryValueInUsd':
      ...
      await setCached('tokenDetails', dataStates.get('tokenDetails')!)
      break
